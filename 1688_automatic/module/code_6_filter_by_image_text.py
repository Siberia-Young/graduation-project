...
def filter_by_image_text(keywords, file_name):
    ...
    try:
        list = []
        def check_keywords_image_text(text, keywords):
        ...
        for row in range(start_row, end_row + 1):
            ...
            for extension in image_extensions:
                ...
                if os.path.exists(image_path):
                    break
            if not sentence is None and (check_keywords_image_text(sentence, keywords) or image_match(base_img, image_path)):
                list.append(row)
