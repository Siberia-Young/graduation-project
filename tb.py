...
def scrape_single_page(driver, keyword, page, file_name):
    ...
    searchUrl = "https://s.taobao.com/search?area=c2c&commend=all&page="+str(page)+"&q="+keyword+"&search_type=mall&sourceId=tb.index&spm=a1z02.1.6856637.d4910789&ssid=s5-e"
    # searchUrl = "https://s.taobao.com/search?area=c2c&commend=all&page=20&q=%E5%B0%8F%E7%B1%B3&search_type=mall&sourceId=tb.index&spm=a1z02.1.6856637.d4910789&ssid=s5-e"
    time.sleep(5)
    ...
    # 使用 select 方法查找指定的元素
    elements = soup.select('div.Content--contentInner--QVTcU0M')
    print(elements)
    # try:
    ...
    #                 goods_img_url = goods_img_urls[0].get('src')
    #                 if goods_img_url:
    #                     goods_img_url = 'https:' + goods_img_url[:-5]
    #                 else:
    #                     goods_img_url = 'https:' + (goods_elements[0].select('img')[0].get('data-lazy-img'))[:-5]
    ...
    #         ordinal_cell = sheet[f"{get_column_letter(last_column)}{last_row}"]
    #         ordinal_cell.alignment = Alignment(wrapText=True, horizontal='center', vertical='center')
    #         sheet.cell(row=last_row, column=last_column, value=element)
    # except Exception as e:
    #     print(e)
    #     driver.quit()
    #     print('与现有浏览器连接断开')
