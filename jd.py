...
def scrape_multiple_pages(keyword, start_page, end_page):
    ...
    headers = ['序号', '监测时间', '店铺经营者', '店铺名称', '店铺链接', '商品ID', '商品标题', '商品链接', '商品首页图', '现价(元)', '销量(评论数)', '销售额(元)', '侵权标签', '搜索关键词', '是否白名单', '处理意见', '备注']
...
def scrape_single_page(driver, keyword, page, file_name):
    ...
    try:
        ...
        for (index, element) in enumerate(elements, start=1):
            ...
            # sheet.column_dimensions[get_column_letter(last_column)].width = len(current_time) / 1.5
            sheet.column_dimensions[get_column_letter(last_column)].width = column_width
            sheet.row_dimensions[last_row].height = row_height
            ...
            sheet.cell(row=last_row, column=last_column, value=current_time)
            ...
            if (len(goods_elements) != 0):
                temp_data = goods_elements[0].get('href')
                # 提取指定部分
                start_index = temp_data.find("//item.jd.com/") + len("//item.jd.com/")
                end_index = temp_data.find(".html")
                goods_id = temp_data[start_index:end_index]
                # sheet.column_dimensions[get_column_letter(last_column)].width = len(goods_id) * 1.1
                sheet.column_dimensions[get_column_letter(last_column)].width = column_width
                sheet.row_dimensions[last_row].height = row_height
                goods_id_cell = sheet[f"{get_column_letter(last_column)}{last_row}"]
                goods_id_cell.alignment = Alignment(wrapText=True, horizontal='center', vertical='center')
            ...
            sheet.column_dimensions[get_column_letter(last_column)].width = column_width
            sheet.row_dimensions[last_row].height = row_height
            tort_tag_cell = sheet[f"{get_column_letter(last_column)}{last_row}"]
            tort_tag_cell.alignment = Alignment(wrapText=True, horizontal='center', vertical='center')
            ...
            sheet.column_dimensions[get_column_letter(last_column)].width = column_width
            sheet.row_dimensions[last_row].height = row_height
            whitelist_orNot_cell = sheet[f"{get_column_letter(last_column)}{last_row}"]
            whitelist_orNot_cell.alignment = Alignment(wrapText=True, horizontal='center', vertical='center')

            # 处理意见
            last_column+=1
            sheet.column_dimensions[get_column_letter(last_column)].width = column_width
            sheet.row_dimensions[last_row].height = row_height
            handling_opinions_cell = sheet[f"{get_column_letter(last_column)}{last_row}"]
            handling_opinions_cell.alignment = Alignment(wrapText=True, horizontal='center', vertical='center')

            # 备注
            last_column+=1
            sheet.column_dimensions[get_column_letter(last_column)].width = column_width
            sheet.row_dimensions[last_row].height = row_height
            remark_cell = sheet[f"{get_column_letter(last_column)}{last_row}"]
            remark_cell.alignment = Alignment(wrapText=True, horizontal='center', vertical='center')
...
if __name__ == "__main__":
    keyword = urllib.parse.quote("华为联名")
    start_page = 1
    end_page = 3
